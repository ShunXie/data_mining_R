...
summary1
mean((test_data$lpsa-y_pred1)^2)
sd((test_data$lpsa-y_pred1)^2)
```
...
print(sprintf("Optimal Lambda: %.3f", optimal_lambda))
...
#Get predictions
y_pred3 <- predict(final_model_cvlasso, newx = x)
mse3 <- mean((test_data$lpsa-y_pred3)^2)
sd3 <- sd((test_data$lpsa-y_pred3)^2)

predict(final_model_cvlasso, s='lambda.min', type='coefficients')
print(mse3)
print(sd3)
```

The model chosen by cv has $\lambda$=0.010. It includes all 8 predictors. The test MSE is 3.662 and standard deviation is 3.198. 
...
#plot cross-validation estimates of MSE
plot(log(lambda_seq),bic_values,type = "l", xlab = "Lambda", ylab = "BIC")

#Get best lambda
optimal_lambda <- lambda_seq[which.min(bic_values)]
print(sprintf("Optimal Lambda: %.3f", optimal_lambda))
...
#Get predictions
y_pred4 <- predict(final_model_biclasso, newx = x)
mse4 <- mean((test_data$lpsa-y_pred4)^2)
sd4 <- sd((test_data$lpsa-y_pred4)^2)

predict(final_model_biclasso, s='lambda.min', type='coefficients')
print(mse4)
print(sd4)
```

The model chosen by bic has $\lambda$=0.254. It only includes 2 predictors, lcavol and lweight. The test MSE is 3.486 and standard deviation is 3.110.
...
#Get predictions
y_pred5 <- predict(pcr_fit, newx = x)
...
It can be seen that model chosen using BIC has a lower number of parameters included in the final model. For example, both best subset method and lasso using BIC criterion choose a model with 2 predictors. On the other hand, both methods using cv choose a full model with 8 predictors. Although training MSE is smaller using cross validation, model chosen by BIC seems to perform better on test data in our case. The final result shows that best subset method using BIC has the best performance on test data, with test MSE 1.536.
